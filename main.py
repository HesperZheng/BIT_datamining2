...
import os
from ResultVisualization import Visualization
import json
...
class Association_rules():
    def __init__(self):
        self.min_sup = min_sup  # 设置最小支持度
        self.min_conf = min_conf  # 设置最小置信度
    ...
    def cal_conf(self, freq_set, H, sup_rata, strong_rules_list):  # 评价指标
        ...
        for reasoned_item in H:
            ...
            jaccard = sup / (sup_rata[freq_set - reasoned_item] + sup_rata[reasoned_item] - sup)  # JACCARD
...
class Oakland_Crime_Statistics():
    ...
    def mining(self, feature_list):
        ...
        # 将数据转为数据字典存储
        dataset = []
        feature_names = ["Agency", "Location", "Area Id", "Beat", "Priority", "Incident Type Id",
...
if __name__ == '__main__':
    ...
    #   可视化时
    #   main函数用以下代码
    #   同时在最顶端引入可视化函数：from ResultVisualization import Visualization
    #———————————————————————————————————————
    # Visualization()
    #——————————————————————————————————————-